...
fun Supplement.toDetailDto(): SupplementDetailDto {
    ...
    val findingsBySymptom = findings.flatMap { f ->
        f.symptoms.map { s -> s to f }
    }.groupBy({ it.first }, { it.second })

    val symptomSections = findingsBySymptom.entries.map { (symptom, sFindings) ->
    ...
    val allSections = if (findingsWithNoSymptom.isNotEmpty()) {
        symptomSections + SupplementSymptomSectionDto(
            symptom = SymptomSummaryDto(id = 0, name = "Other", slug = "other"),
            pairSummary = null,
            evidenceStrength = null,
            findings = findingsWithNoSymptom.map { it.toDto() }
        )
    } else symptomSections
...
fun SymptomDetail.toResponseDto(): SymptomDetailResponseDto {
    val supplementSections = supplements.map { sup ->
        val pairForThisSymptom = sup.pairSummaries.firstOrNull { it.symptomId == symptom.id }
        val findingsForSymptom = sup.findings.filter { f ->
