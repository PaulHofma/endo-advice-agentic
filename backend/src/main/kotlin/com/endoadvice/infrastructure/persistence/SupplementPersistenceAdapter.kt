...
class SupplementPersistenceAdapter(
    ...
    override fun findAll(): List<Supplement> = supplementJpaRepository.findAllWithFindings().map { it.toDomain() }

    @Transactional(readOnly = true)
    override fun findById(id: Long): Supplement? = supplementJpaRepository.findById(id).orElse(null)?.toDomainWithFindings()
}

private fun SupplementEntity.toDomain() = Supplement(id = id, name = name, summary = summary)

private fun SupplementEntity.toDomainWithFindings() =
    Supplement(
        id = id,
        name = name,
        summary = summary,
