...
class SymptomPersistenceAdapter(
    ...
    override fun findAll(): List<Symptom> =
        symptomJpaRepository.findAllWithFindings().map { it.toDomain() }

    override fun findBySlug(slug: String): Symptom? =
        symptomJpaRepository.findBySlug(slug)?.toDomain()
}
